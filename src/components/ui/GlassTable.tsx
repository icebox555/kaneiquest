...
export function GlassTable<T extends Record<string, any>>({ data, columns, keyField, isLoading }: GlassTableProps<T>) {
    if (isLoading) {
        return (
            <div className="admin-glass rounded-xl p-8 flex justify-center items-center">
    ...
    if (data.length === 0) {
        return (
            <div className="admin-glass rounded-xl p-8 text-center text-slate-400">
                データがありません
            </div>
        );
    }
                        ...
                        {data.map((item, rowIndex) => (
                            ...
                            >
                                {columns.map((col, colIndex) => (
                                    <td key={colIndex} className={`px-6 py-4 text-slate-300 ${col.className || ""}`}>

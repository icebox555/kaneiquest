...
export function ExamCountdown({ examDate, dailyGoalMinutes }: Props) {
    ...
    return (
        <div className={`glass p-6 rounded-2xl space-y-3 border ${isNear ? "border-rose-300/50 bg-rose-50/20" : "border-transparent"}`}>
            <div className="flex items-center gap-3">
                <div className={`p-3 rounded-xl ${isNear ? "bg-rose-500/10 text-rose-500" : "bg-primary/10 text-primary"}`}>
                    <CalendarDays className="w-6 h-6" />
                </div>
                <div>
                    <h3 className="text-lg font-bold text-stone-800">試験まであと</h3>
            ...
            {isPast ? (
                ...
                <>
                    <div className="flex items-end gap-2">
                        <span className={`text-5xl font-extrabold ${isNear ? "text-rose-500" : "text-gradient-gold"}`}>
                            {daysLeft}
                        </span>
                        <span className="text-stone-500 mb-2 text-lg">日</span>
                    </div>
